    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-aloja-navy">{t('housekeeping.checklists.title')}</h1>
          <p className="mt-1 text-xs text-aloja-gray-800/70">
            {t('housekeeping.config.manage_checklists_desc')}
          </p>
        </div>
        <Button variant="primary" size="md" onClick={() => { setEditChecklist(null); setShowModal(true); }}>
          {t('housekeeping.checklists.new_checklist')}
        </Button>
      </div>
          ...
          { key: 'is_default', header: t('housekeeping.checklists.is_default'), sortable: true, render: (r) => r.is_default ? t('common.yes') : t('common.no') },
          {
            ...
            render: (r) => (
              <div className="flex justify-end items-center gap-x-2">
                <EditIcon size="18" onClick={() => { setEditChecklist(r); setShowModal(true); }} className="cursor-pointer" />
                <DeleteButton resource="housekeeping/checklists" id={r.id} onDeleted={refetch} />
              </div>
            )
          },
        ]}
