...
const Item = ({ to, children, onMobileClose, isMobile, exact = false }) => (
  <NavLink
    ...
    className={({ isActive }) =>
      `relative overflow-hidden flex items-center gap-3 h-10 px-4 text-sm rounded-md transition-colors ${
  ...
  >
    {({ isActive }) => (
      <>
        ...
        <span
          aria-hidden
          className={`pointer-events-none absolute left-0 top-0 h-full bg-white/5 rounded-md transition-[width] duration-250 ease-out ${
            isActive ? "w-full" : "w-0"
          }`}
          style={{ zIndex: 0 }}
        />
        ...
        <span className="inline-flex items-center gap-2" style={{ zIndex: 1 }}>
          {children}
        </span>
      </>
    )}
...
export default function Sidebar({
  ...
  const hasReception = useHasAnyPermission([
  ...
  const hasViewInvoicing = useHasAnyPermission([
    "invoicing.view_invoice",
    "invoicing.view_receipt",
  ]);
  const hasViewBankReconciliation = usePermissions(
    "payments.view_bankreconciliation"
  );
  const hasViewCashbox = useHasAnyPermission([
    "cashbox.view_cashsession",
    "cashbox.view_cashbox_reports",
    "cashbox.open_cashsession",
    "cashbox.close_cashsession",
  ]);
  const hasAnyFinancial = useHasAnyPermission([
    "payments.view_refund",
    "payments.view_refundvoucher",
    "invoicing.view_invoice",
    "invoicing.view_receipt",
    "payments.view_bankreconciliation",
    "cashbox.view_cashsession",
  ...
  const hasAnyLocation = useHasAnyPermission([
  ...
  const hasAnyRate = useHasAnyPermission([
    "rates.view_rateplan",
    "rates.view_raterule",
  ...
  const canShowHousekeepingTasks =
    housekeepingEnabled && (hasViewHousekeeping || isHKStaff || isSuperuser);
  const canShowHousekeepingConfig =
    housekeepingEnabled && (hasHousekeepingConfig || isSuperuser);
  ...
  const { results: hkSidebarConfig } = useAction({
    ...
    action:
      hasSingleHotel && singleHotelId ? `by-hotel/${singleHotelId}` : undefined,
    enabled:
      hasSingleHotel &&
      !!singleHotelId &&
      hasHousekeepingConfig &&
      canShowHousekeepingConfig,
  });

  const canSeeHousekeepingChecklistsMenu =
    hasHousekeepingConfig && hkSidebarConfig?.use_checklists !== false;
  ...
  useEffect(() => {
    ...
    const isHousekeeping = location.pathname.startsWith(
  ...
  return (
    ...
    >
      ...
      <div
        className="flex flex-col p-3 gap-2 overflow-y-auto no-scrollbar h-full relative"
        style={{ zIndex: 1 }}
      >
        ...
        <nav className="mt-3 flex flex-col gap-1">
          ...
          {isOnlyHousekeepingStaff ? (
            <Item
              ...
              exact={true}
            >
              <CleaningIcon size="20" />{" "}
              {!isMini && <span>{t("sidebar.housekeeping")}</span>}
            </Item>
          ) : (
            <>
              {hasViewDashboard && (
                <Item to="/" onMobileClose={onMobileClose} isMobile={isMobile}>
                  <DashboardIcon size="20" />{" "}
                  {!isMini && <span>{t("sidebar.dashboard")}</span>}
                </Item>
              )}
              {hasReception && (
                <Item
                  ...
                  isMobile={isMobile}
                >
                  <ReceptionIcon size="20" />{" "}
                  {!isMini && <span>{t("sidebar.reception")}</span>}
                </Item>
              )}
              {hasViewReservations && (
                <Item
                  to="/reservations-gestion"
                  onMobileClose={onMobileClose}
                  isMobile={isMobile}
                >
                  <BellIcon size="20" />{" "}
                  {!isMini && (
                    <span>{t("sidebar.reservations_management")}</span>
                  )}
                </Item>
              )}
              {hasViewCalendar && (
                ...
                >
                  ...
                  {!isMini && <span>Calendario de Reservas</span>}
                </Item>
              )}
              ...
              {hasViewRooms && (
                ...
                >
                  <RoomsIcon size="20" />{" "}
                  {!isMini && <span>{t("sidebar.rooms_management")}</span>}
                </Item>
              )}
              {hasViewOTAs && (
                <Item
                ...
                >
                  ...
                  {!isMini && <span>{t("sidebar.channels")}</span>}
                </Item>
              )}
            </>
          )}
            ...
            <div className="mt-1">
              ...
              >
                {hasViewReservationsHistory && (
                  <Item
                    to="/reservations/1/history"
                    onMobileClose={onMobileClose}
                    isMobile={isMobile}
                  >
                    {t("sidebar.reservations_history")}
                  </Item>
                )}
                {hasViewPayments && (
                  <Item
                ...
                {hasViewRefunds && (
                  <Item
                    ...
                    isMobile={isMobile}
                  >
                    {t("sidebar.refunds_history")}
                  </Item>
                )}
                {canShowHousekeepingTasks && (
                  <Item
                    to="/housekeeping/historical"
                    onMobileClose={onMobileClose}
                    isMobile={isMobile}
                  >
                    {t("sidebar.housekeeping_historical")}
                  </Item>
                )}
              </div>
            </div>
              ...
              >
                    ...
                    >
                      {hasViewInvoices && (
                        <Item
                          ...
                          exact={true}
                        >
                          {t("sidebar.invoicing_electronic")}
                        </Item>
                      )}
                      {hasViewReceipts && (
                        <Item
                ...
                {hasViewBankReconciliation && (
                  <Item
                    ...
                    isMobile={isMobile}
                  >
                    {!isMini && <span>{t("sidebar.bank_reconciliation")}</span>}
                  </Item>
                )}
              </div>
            </div>
                  ...
                  {hasAnyRate && (
                    <div className="mt-1 ml-2">
                      ...
                      >
                        {hasViewRatePlans && (
                          <Item
                            to="/settings/rates/plans"
                            onMobileClose={onMobileClose}
                            isMobile={isMobile}
                          >
                            {t("sidebar.rate_plans")}
                          </Item>
                        )}
                        {hasViewRateRules && (
                          <Item
                            to="/settings/rates/rules"
                            onMobileClose={onMobileClose}
                            isMobile={isMobile}
                          >
                            {t("sidebar.rate_rules")}
                          </Item>
                        )}
                        {hasViewPromos && (
                          <Item
                            to="/settings/rates/promos"
                            onMobileClose={onMobileClose}
                            isMobile={isMobile}
                          >
                    ...
                    <div className="mt-1 ml-2">
                      ...
                      >
                        {hasViewPaymentPolicies && (
                          <Item
                            to="/settings/payments/policies"
                            onMobileClose={onMobileClose}
                            isMobile={isMobile}
                          >
                            {t("sidebar.payment_policies")}
                          </Item>
                        )}
                        {hasViewCancellationPolicies && (
                          <Item
                            to="/settings/policies/cancellation"
                            onMobileClose={onMobileClose}
                            isMobile={isMobile}
                          >
                            {t("sidebar.cancellation_policies")}
                          </Item>
                        )}
                        {hasViewRefundPolicies && (
                          <Item
                            to="/settings/policies/devolution"
                            onMobileClose={onMobileClose}
                            isMobile={isMobile}
                          >
                            {t("sidebar.devolution_policies")}
                          </Item>
                        )}
                      </div>
                    </div>
