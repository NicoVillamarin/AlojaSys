...
class Command(BaseCommand):
    help = 'Prueba la funcionalidad de conciliacion bancaria (version minimal)'

    def handle(self, *args, **options):
        self.stdout.write('INICIANDO PRUEBA DE CONCILIACION BANCARIA (MINIMAL)')
        self.stdout.write('=' * 60)

        # 1. Crear hotel de prueba (sin enterprise)
        ...
        try:
            hotel = Hotel.objects.create(
                ...
                enterprise=None  # Sin enterprise
            )
            self.stdout.write(f'[OK] Hotel creado: {hotel.name}')
        except Exception as e:
            self.stdout.write(f'[ERROR] Error creando hotel: {e}')
            return
        ...
        for i, res_data in enumerate(test_reservations):
            try:
                ...
                self.stdout.write(f'   [ERROR] Error creando reserva {res_data["guest_name"]}: {e}')
        ...
        try:
            ...
            self.stdout.write(f'[ERROR] Error creando conciliacion: {e}')
            return
        ...
        for trans_data in test_transactions:
            try:
                ...
                self.stdout.write(f'   [ERROR] Error creando transaccion {trans_data["description"]}: {e}')
        ...
        for transaction in created_transactions:
            try:
                ...
                self.stdout.write(f'   [ERROR] Error creando match para {transaction.description}: {e}')
        ...
        try:
            ...
            self.stdout.write(f'[ERROR] Error actualizando estadisticas: {e}')
        ...
        try:
            ...
            self.stdout.write(f'[ERROR] Error limpiando datos: {e}')
