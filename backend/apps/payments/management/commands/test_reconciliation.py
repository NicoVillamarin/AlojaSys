...
class Command(BaseCommand):
    help = 'Prueba la funcionalidad de conciliación bancaria'
    ...
    def handle(self, *args, **options):
        ...
        for i, res_data in enumerate(test_reservations):
            ...
            self.stdout.write(f'   ✅ {res_data["guest_name"]}: ${res_data["amount"]}')

        self.stdout.write(f'\n📊 Estado ANTES de la conciliación:')
        self.stdout.write(f'   - Pagos pendientes: {Payment.objects.filter(status="pending").count()}')

        # 3. Crear conciliación
        self.stdout.write('\n🔄 Creando conciliación bancaria...')
        ...
        self.stdout.write(f'✅ Conciliación creada: #{reconciliation.id}')
        ...
        for trans_data in test_transactions:
            ...
            self.stdout.write(f'   ✅ {trans_data["description"]}: ${trans_data["amount"]}')

        # 5. Simular matching automático
        self.stdout.write('\n🔗 Simulando matching automático...')
        matches_created = 0
        ...
        for transaction in created_transactions:
            ...
            if matching_payment:
                ...
                matches_created += 1
                self.stdout.write(f'   ✅ Match: {transaction.description} → {matching_payment.reservation.guest_name}')
        ...
        self.stdout.write(f'   - Efectividad: {reconciliation.match_percentage:.1f}%')
        self.stdout.write(f'   - La funcionalidad de conciliación bancaria está funcionando correctamente!')
