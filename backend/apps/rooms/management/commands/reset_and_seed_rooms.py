...
class Command(BaseCommand):
    help = "Borra todas las reservas y habitaciones, luego crea 30 habitaciones completas para cada hotel."

    def add_arguments(self, parser):
        parser.add_argument("--confirm", action="store_true", help="Confirma que quieres borrar todos los datos")
    ...
    def handle(self, *args, **options):
        if not options["confirm"]:
            self.stdout.write(
                self.style.WARNING(
                    "⚠️  ADVERTENCIA: Este comando borrará TODAS las reservas y habitaciones.\n"
        ...
        # 1. Borrar todas las reservas y bloqueos primero
        self.stdout.write("🗑️  Borrando todas las reservas...")
        reservations_deleted = Reservation.objects.all().delete()[0]
        self.stdout.write(f"   ✅ {reservations_deleted} reservas eliminadas")
        
        self.stdout.write("🗑️  Borrando todos los bloqueos de habitaciones...")
        blocks_deleted = RoomBlock.objects.all().delete()[0]
        self.stdout.write(f"   ✅ {blocks_deleted} bloqueos eliminados")
        
        # 2. Borrar todas las habitaciones
        ...
        # 4. Crear habitaciones para cada hotel
        total_created = 0
        ...
        self.stdout.write(
            self.style.SUCCESS(
                ...
                f"   • {reservations_deleted} reservas eliminadas\n"
                f"   • {blocks_deleted} bloqueos eliminados\n"
    ...
    def _create_rooms_for_hotel(self, hotel, count):
        ...
        # Distribución de pisos (más habitaciones en pisos bajos)
